...
def write_consumption_csv(path: Path, metadata: dict[str, str], rows: list[dict[str, str]]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as file:
        ...
        writer = csv.DictWriter(file, fieldnames=["date", "start", "end", "consumption_kwh"])
        writer.writeheader()
...
def export_directory(hass: HomeAssistant, directory: str) -> tuple[Path, str]:
    ...
    if relative.is_absolute() or ".." in relative.parts:
        raise HomeAssistantError("directory must be a relative path below config/www")
...
async def async_export_electricity_consumption_csv(
    ...
    try:
        ...
        raise HomeAssistantError("start_date and end_date must use YYYY-MM-DD") from err
    if end_date < start_date or (end_date - start_date).days > 30:
        raise HomeAssistantError("The export period must be between 1 and 31 days")
    ...
    if len(supplies) > 1:
        raise HomeAssistantError("Set supply_point_id when more than one electricity supply is available")
    ...
    if not rows:
        raise HomeAssistantError("No consumption data was available for the selected period")
